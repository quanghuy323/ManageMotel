...
public class SearchFloorServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String searchValueValid = "([0-9]){0,}";
        ErrorMessage error = new ErrorMessage();
        boolean bError = false;
        String url = searchPage;
        try {

            if (!searchValue.trim().matches(searchValueValid)) {
                error.setSearchError("Please input floor with a NUMBER !!!");
                bError = true;
            }
            ...
            if (bError) {
                ...
                dao.searchRoom(searchValue);
                List<RoomDTO> listRoom = dao.getListRoom();
                request.setAttribute("SEARCHRESULT", listRoom);
            }
        } catch (NamingException ex) {
            log("SearchFloorServlet - Naming: " + ex.getMessage());
        } catch (SQLException ex) {
            log("SearchFloorServlet - SQL: " + ex.getMessage());
        } finally {
